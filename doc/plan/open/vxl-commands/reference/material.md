...
the same input, object selection, and atlas mode would produce, and they
line up with that mesh's UVs. With `--atlas palette` the object's layers merge
per property name by the format's layer-override resolution and the maps hold
one texel per distinct flattened material the object uses; with `--atlas
unwrap` they are unwrapped per mesh. Either way you can iterate on materials
without re-meshing.
