use crate::{TyPose, TyUniformTrs};
use glam::{DQuat, DVec3};

/// A node transform composing as `Translation * Rotation * Scale`. glam has no
/// lossless TRS-with-per-axis-scale, so this stays a hand-rolled struct whose
/// bodies run on glam ops. The bare name is the `f64` form.
#[derive(Clone, Copy, Debug, PartialEq)]
pub struct TyTransform {
...
impl TyTransform {
    ...
    /// This transform as a [`TyUniformTrs`], taking `scale.x` as the uniform
    /// factor. Assumes the scale is uniform.
    pub fn to_uniform_trs(&self) -> TyUniformTrs {
        TyUniformTrs::new(self.position, self.rotation, self.scale.x)
    }
}

impl Default for TyTransform {
    fn default() -> Self {
        Self::IDENTITY
    }
}
