use crate::{Result, vox_state_from_voxj_codec_main};
...
pub fn vox_state_from_voxj_bytes(bytes: &[u8]) -> Result<VoxState> {
    let serde_file = from_voxj_or_voxjz_file_bytes(bytes)?;
    let codec_file = decode_voxj_file(&serde_file)?;
