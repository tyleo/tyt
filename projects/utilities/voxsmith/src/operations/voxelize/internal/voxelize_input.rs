use crate::{
    Result,
    operations::voxelize::{
        GridResolution, MeshInput, ResolutionAxis, VoxelizeOptions, voxelize_mesh,
    },
...
/// Voxelizes `input` under `options` into a [`VoxMain`] of one object placed
/// by one root node, the body of
/// [`voxelize`](crate::operations::voxelize::voxelize()) once the document
/// is flattened. `fallback_name` names the object when neither
/// `options.name` nor the input does.
pub fn voxelize_input(
    input: &MeshInput,
    fallback_name: &str,
...
mod tests {
    ...
    use crate::{
        operations::voxelize::{
            FillMode, GridResolution, MaterialMode, MeshInput, MeshMaterial, MeshTriangle,
            MeshTriangleUvs, OutOfRangeProperty, ResolutionAxis, SurfaceMode, VoxelizeOptions,
            voxelize_input,
        },
    ...
    /// cell.
    fn cells_input(colors: &[TyLinSrgbaF64]) -> MeshInput {
    ...
    fn a_reduction_caps_the_generated_palette_and_the_state_stays_valid() {
        ...
        let main = voxelize_input(&input, "fallback", &options(Some(reduction))).unwrap();
        assert_eq!(main.validate(), Ok(()));
    ...
    fn the_name_override_beats_the_mesh_name_and_the_fallback_fills_in() {
        let input = cells_input(&[TyLinSrgbaF64::new(1.0, 0.0, 0.0, 1.0)]);

        let named = VoxelizeOptions {
            name: Some("override".to_owned()),
            ..options(None)
        };
        let main = voxelize_input(&input, "fallback", &named).unwrap();
        assert_eq!(main.iter_objects().next().unwrap().1.name(), "override");

        let mut nameless = cells_input(&[TyLinSrgbaF64::new(1.0, 0.0, 0.0, 1.0)]);
        nameless.name = None;
        let main = voxelize_input(&nameless, "fallback", &options(None)).unwrap();
