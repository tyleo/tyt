...
pub fn mesh_old<D: EncodePng, T: VoxExt>(
    dependencies: &D,
    main: &VoxMain<T>,
    object: &VoxObject,
...
mod tests {
    use crate::{
        ...
        operations::mesh_old::{
            ...
            MaterialSlot, MeshMethod, mesh_old,
        },
    };
    use branded_id::U32Id;
    use meshdoc::{
    ...
    fn an_empty_object_meshes_to_an_object_with_no_primitive() {
        ...
        let document =
            mesh_old(&DependenciesImpl, &main, &empty, &request(Vec::new(), 1.0)).unwrap();
