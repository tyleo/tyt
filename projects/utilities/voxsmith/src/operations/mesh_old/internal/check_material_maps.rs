use crate::{
    Error, Result,
    operations::mesh_old::{ColorChannel, MaterialBake, MaterialChannel, MaterialMap},
};
use voxcore::{
    VoxEffectivePalette, VoxExt, VoxMain, VoxObject, VoxValuePool, VoxValuePoolKind,
...
pub(crate) fn check_material_maps<T: VoxExt>(
    main: &VoxMain<T>,
    object: &VoxObject,
    maps: &[MaterialMap],
) -> Result<()> {
...
mod tests {
    ...
    use crate::operations::mesh_old::ColorChannel;
    ...
    fn palette_main() -> (VoxMain, U32Id<BVoxPalette>) {
        ...
        let glow_value_pool_id =
            main.retain_value_pool(VoxValuePool::vec_3_float(vec![[0.0, 1.0, 0.0]]).unwrap());
        let gloss_value_pool_id = main.retain_value_pool(VoxValuePool::float(vec![0.5]).unwrap());
    ...
    fn checks(
        ...
        let effective = main.effective_palette(&object).unwrap();
        check_channel(&effective, key, component).is_ok()
    }

    #[test]
    fn a_present_color_reads_by_component() {
        let (main, palette_id) = palette_main();
        // `tint` is a vec-4-float value pool: a component is required, and `A`
        // is allowed.
        assert!(!checks(&main, &[palette_id], "tint", None));
        assert!(checks(&main, &[palette_id], "tint", Some(ColorChannel::R)));
        assert!(checks(&main, &[palette_id], "tint", Some(ColorChannel::A)));
    }

    #[test]
    fn a_present_three_component_color_rejects_alpha() {
        let (main, palette_id) = palette_main();
        assert!(checks(&main, &[palette_id], "glow", Some(ColorChannel::B)));
        assert!(!checks(&main, &[palette_id], "glow", Some(ColorChannel::A)));
    }

    #[test]
    fn a_present_scalar_rejects_a_component() {
        let (main, palette_id) = palette_main();
        assert!(checks(&main, &[palette_id], "gloss", None));
        assert!(!checks(
            &main,
            &[palette_id],
            "gloss",
    ...
    fn an_absent_builtin_takes_its_spec_kind() {
        ...
        assert!(!checks(&main, &[palette_id], "baseColor", None));
        assert!(checks(&main, &[palette_id], "occlusionStrength", None));
        assert!(!checks(
            &main,
            &[palette_id],
            "occlusionStrength",
    ...
    fn a_bound_builtin_takes_its_vocabulary_kind() {
        ...
        assert!(checks(&main, &[palette_id], "metallic", None));
        assert!(!checks(
            &main,
            &[palette_id],
            "metallic",
    ...
    fn an_absent_custom_property_is_an_error() {
        ...
        assert!(!checks(&main, &[palette_id], "subsurface", None));
        assert!(!checks(
            &main,
            &[palette_id],
            "subsurface",
    ...
    fn checks_whole(main: &VoxMain, palette_ids: &[U32Id<BVoxPalette>], key: &str) -> (bool, bool) {
        let mut object = VoxObject::new("body".to_owned(), TyVector3U32::new(1, 1, 1)).unwrap();
        for &palette_id in palette_ids {
            object.retain_layer(palette_id, U32Id::from_u32(0));
        }
    ...
    fn a_whole_read_property_is_checked_against_the_kind_it_is_read_as() {
        ...
        let (main, palette_id) = palette_main();
        assert_eq!(checks_whole(&main, &[palette_id], "tint"), (true, false));
        assert_eq!(checks_whole(&main, &[palette_id], "gloss"), (false, true));
    }
    ...
    fn a_string_value_pool_has_no_texel_value() {
        ...
        let tag_value_pool_id =
            main.retain_value_pool(VoxValuePool::string(vec!["low".to_owned()]));
        let mut palette = VoxPalette::default();
        palette
        ...
        assert!(!checks(&main, &[palette_id], "tag", None));
    }
    ...
    fn an_int_vector_value_pool_has_no_texel_value() {
        ...
        let cell_value_pool_id =
            main.retain_value_pool(VoxValuePool::vec_3_int(vec![[1, 2, 3]]).unwrap());
        let mut palette = VoxPalette::default();
        palette
        ...
        assert!(!checks(&main, &[palette_id], "cell", None));
        assert!(!checks(&main, &[palette_id], "cell", Some(ColorChannel::R)));
    }
    ...
    fn a_key_takes_its_winning_layers_kind() {
        ...
        let mut main = VoxMain::default();
        let scalar_value_pool_id = main.retain_value_pool(VoxValuePool::float(vec![0.5]).unwrap());
        ...
        let scalar_palette_id = main.retain_palette(scalar_palette).unwrap();
        ...
        let color_palette_id = main.retain_palette(color_palette).unwrap();
