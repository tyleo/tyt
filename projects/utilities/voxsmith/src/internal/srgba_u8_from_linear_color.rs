...
#[cfg(test)]
mod tests {
    use super::srgba_u8_from_linear_color;
    use crate::linear_color_from_srgba_u8;
    ...
    fn every_u8_code_round_trips_through_linear() {
        for code in 0..=255u8 {
            ...
            let round_tripped = srgba_u8_from_linear_color(linear_color_from_srgba_u8(color));
    ...
    fn decode_matches_the_reference_transfer() {
        for code in 0..=255u8 {
            ...
            let linear = linear_color_from_srgba_u8(TySrgbaU8::new(code, 0, 0, 255)).red;
            assert!(
    ...
    fn alpha_is_never_transfer_encoded() {
        for code in [0u8, 1, 64, 128, 191, 254, 255] {
            let alpha = linear_color_from_srgba_u8(TySrgbaU8::new(0, 0, 0, code)).alpha;
