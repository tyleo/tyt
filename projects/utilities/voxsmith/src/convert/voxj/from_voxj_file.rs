use crate::{
    Error, Result, VoxjExtSlot, vox_hierarchy_node_from_voxj_hierarchy_node,
...
pub fn from_voxj_file<T: VoxjExtSlot>(file: &VoxjFile) -> Result<VoxMain<T>> {
    ...
    state.set_ext(T::from_voxj_ext(main.ext.as_ref())?);
...
mod tests {
    use crate::{
        EditStateMode, VoxjFileBuilder, VoxjRawExt, VoxjRawVoxMain, from_voxj_bytes,
        from_voxj_file, to_voxj_bytes, to_voxj_file, to_voxjz_bytes,
    };
    use branded_id::U32Id;
    ...
    fn round_trips_through_vox_state() {
        ...
        let state: VoxjRawVoxMain = from_voxj_file(&file).unwrap();
    ...
    fn builder_default_matches_to_voxj_file() {
        let state: VoxjRawVoxMain = from_voxj_file(&sample_file()).unwrap();
        assert_file_eq(
    ...
    fn round_trips_through_voxj_bytes() {
        ...
        let reloaded: VoxjRawVoxMain = from_voxj_bytes(&bytes).unwrap();
    ...
    fn gc_on_a_loaded_state_preserves_the_round_trip() {
        let file = sample_file();
        let mut state: VoxjRawVoxMain = from_voxj_file(&file).unwrap();
    ...
    fn remove_then_gc_round_trips_through_bytes() {
        let mut state: VoxjRawVoxMain = from_voxj_file(&sample_file()).unwrap();
    ...
    fn rejects_position_outside_bounds() {
        ...
        assert!(from_voxj_file::<Option<VoxjRawExt>>(&file).is_err());
    }
