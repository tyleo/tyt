...
/// [`vox_state_from_vmax_file`](crate::vox_state_from_vmax_file).
...
pub fn vmax_file_from_vox_state(
    state: &VoxState,
...
#[cfg(test)]
mod tests {
    use crate::{VoxelMaxColorFormat, vmax_file_from_vox_state, vox_state_from_vmax_file};
    ...
    fn round_trips_through_vox_state() {
        let original = sample();
        let state = vox_state_from_vmax_file(&original).unwrap();
        let rebuilt = vmax_file_from_vox_state(&state, VoxelMaxColorFormat::Png).unwrap();
    ...
    fn errors_without_voxel_max_ext() {
        ...
        assert!(vmax_file_from_vox_state(&state, VoxelMaxColorFormat::Png).is_err());
    }
}
