use crate::{
    Error, PaletteRef, PaletteShowLabel, PaletteShowLayout, PaletteShowOptions,
    PaletteShowPresentation, PaletteShowReading, PaletteShowTableShape, PropertyRef,
    PropertySelector, Result, VectorComponent, property_names,
};
...
mod tests {
    use crate::{
        ...
        PaletteShowPresentation, PaletteShowReading, PaletteShowTableShape, PropertyRef,
        PropertySelector, VectorComponent, render_palette_show,
        report::palette_show::render_palette_show::resolve_value_collections,
    };
    use branded_id::U32Id;
    ...
    fn show_with(
        ...
        render_palette_show(state, &selectors(fields), &options).unwrap()
    }
    ...
    fn rows_wrap_cells_to_the_width() {
        ...
        let output = render_palette_show(
            &state,
    ...
    fn a_label_mode_on_the_hierarchy_layout_is_invalid_input() {
        let state = sample_state();
        let result = render_palette_show(
            &state,
            &selectors(&[("0", "baseColor", "value", "auto")]),
            &PaletteShowOptions {
