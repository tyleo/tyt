...
//! [`vox_state_to_voxjz_bytes`]). For Voxel Max it converts to and from the
//! lossless `VMaxFile` ([`vox_state_from_vmax_file`] /
//! [`vmax_file_from_vox_state`]), stashing the Voxel Max state with no
