...
pub struct MeshOldPropertyBinding {
    name: String,
    key: String,
}

impl MeshOldPropertyBinding {
...
#[cfg(test)]
mod tests {
    use crate::commands::MeshOldPropertyBinding;

    #[test]
    fn pairs_a_property_and_name() {
        let binding = MeshOldPropertyBinding::new("gloss", "roughness").unwrap();
    ...
    fn keeps_the_name_verbatim_including_spaces() {
        ...
        assert_eq!(
            MeshOldPropertyBinding::new("emissive", "super emissive thing")
                .unwrap()
                .key(),
            "super emissive thing"
        );
        assert_eq!(
            MeshOldPropertyBinding::new("tint", "tint:extra")
                .unwrap()
                .key(),
            "tint:extra"
        );
    ...
    fn rejects_an_empty_side() {
        assert!(MeshOldPropertyBinding::new("", "tint").is_err());
        assert!(MeshOldPropertyBinding::new("tint", "").is_err());
    }
    ...
    fn rejects_whitespace_in_the_property() {
        ...
        assert!(MeshOldPropertyBinding::new("super gloss", "roughness").is_err());
    }
}
