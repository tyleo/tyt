use crate::CliValue;
use voxsmith::{
    operations::mesh_old::{ColorChannel, MaterialChannel},
...
pub fn parse_mesh_old_material_channel(text: &str) -> Result<MaterialChannel, String> {
    match text {
...
#[cfg(test)]
mod tests {
    use crate::commands::parse_mesh_old_material_channel;
    ...
    fn parses_constants_and_computed_occlusion() {
        assert_eq!(
            parse_mesh_old_material_channel("0").unwrap(),
            MaterialChannel::Zero
        );
        assert_eq!(
            parse_mesh_old_material_channel("1").unwrap(),
            MaterialChannel::One
        );
        assert_eq!(
            parse_mesh_old_material_channel("computed-occlusion").unwrap(),
    ...
    fn parses_property_and_inverse() {
        assert_eq!(
            parse_mesh_old_material_channel("metallic").unwrap(),
            property(METALLIC, false)
        );
        assert_eq!(
            parse_mesh_old_material_channel("1-metallic").unwrap(),
    ...
    fn parses_inverted_roughness() {
        assert_eq!(
            parse_mesh_old_material_channel("roughness").unwrap(),
            property(ROUGHNESS, false)
        );
        assert_eq!(
            parse_mesh_old_material_channel("1-roughness").unwrap(),
    ...
    fn parses_components_from_either_alias_set() {
        assert_eq!(
            parse_mesh_old_material_channel("baseColor.r").unwrap(),
            component(BASE_COLOR, ColorChannel::R, false)
        );
        assert_eq!(
            parse_mesh_old_material_channel("1-baseColor.a").unwrap(),
            component(BASE_COLOR, ColorChannel::A, true)
        );
        assert_eq!(
            parse_mesh_old_material_channel("normal.x").unwrap(),
            component("normal", ColorChannel::R, false)
        );
        assert_eq!(
            parse_mesh_old_material_channel("normal.w").unwrap(),
    ...
    fn keeps_dotted_keys_without_a_component() {
        assert_eq!(
            parse_mesh_old_material_channel("my.attr").unwrap(),
    ...
    fn rejects_empty_property() {
        assert!(parse_mesh_old_material_channel("").is_err());
        assert!(parse_mesh_old_material_channel("1-").is_err());
    }

    #[test]
    fn rejects_an_unknown_component() {
        assert!(parse_mesh_old_material_channel("baseColor.q").is_err());
    }
    ...
    fn rejects_a_whitespace_property() {
        ...
        assert!(parse_mesh_old_material_channel("super emissive thing").is_err());
        assert!(parse_mesh_old_material_channel("1-super emissive thing").is_err());
    }
}
