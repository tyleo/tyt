use crate::{
    Result,
    commands::{
        MeshOldPropertyBinding, parse_mesh_old_material_channel, resolve_mesh_old_material_channel,
    },
};
...
pub struct MeshOldChannelPacking {
...
impl MeshOldChannelPacking {
    ...
    pub(crate) fn resolve(&self, bindings: &[MeshOldPropertyBinding]) -> Result<MaterialBake> {
        let channels = self
            .sources()
            .iter()
            .map(|channel| resolve_mesh_old_material_channel(channel, bindings))
...
impl FromStr for MeshOldChannelPacking {
    type Err = String;
    ...
    fn from_str(value: &str) -> StdResult<Self, Self::Err> {
        let mut packing = MeshOldChannelPacking {
            r: None,
            g: None,
            b: None,
            a: None,
        ...
        for part in value.split(',') {
            ...
            let source = parse_mesh_old_material_channel(expr)?;
...
mod tests {
    ...
    fn parses_single_channel() {
        let packing = "R=computed-occlusion"
            .parse::<MeshOldChannelPacking>()
            .unwrap();
    ...
    fn rejects_bad_packings() {
        assert!(
            "R=metallic,R=roughness"
                .parse::<MeshOldChannelPacking>()
                .is_err()
        );
        assert!("X=metallic".parse::<MeshOldChannelPacking>().is_err());
        assert!("metallic".parse::<MeshOldChannelPacking>().is_err());
        assert!("".parse::<MeshOldChannelPacking>().is_err());
    }
}
