use crate::{
    Result,
    commands::{MeshOldPropertyBinding, mesh_old_computed_occlusion_unsupported},
};
use voxsmith::operations::mesh_old::MaterialChannel;
...
pub(crate) fn resolve_mesh_old_material_channel(
    channel: &MaterialChannel,
    bindings: &[MeshOldPropertyBinding],
...
#[cfg(test)]
mod tests {
    use crate::commands::{MeshOldPropertyBinding, resolve_mesh_old_material_channel};
    use voxcore::material::{BASE_COLOR, METALLIC, ROUGHNESS};
    use voxsmith::operations::mesh_old::{ColorChannel, MaterialChannel};
    ...
    fn bindings() -> Vec<MeshOldPropertyBinding> {
        vec![
            MeshOldPropertyBinding::new("gloss", ROUGHNESS).unwrap(),
            MeshOldPropertyBinding::new("tint", "tint").unwrap(),
        ]
    }
    ...
    fn a_binding_resolves_to_its_concrete_key() {
        ...
        let resolved =
            resolve_mesh_old_material_channel(&property("gloss", true), &bindings()).unwrap();
        ...
        let resolved = resolve_mesh_old_material_channel(
            &component("tint", ColorChannel::R, false),
            &bindings(),
        )
        .unwrap();
    ...
    fn a_binding_reaches_a_name_with_spaces() {
        ...
        // name, spaces and all.
        let bindings = vec![MeshOldPropertyBinding::new("spark", "super emissive thing").unwrap()];
        let resolved =
            resolve_mesh_old_material_channel(&property("spark", false), &bindings).unwrap();
    ...
    fn resolve_does_not_check_the_component() {
        ...
        assert!(
            resolve_mesh_old_material_channel(
                &component(METALLIC, ColorChannel::R, false),
                &bindings()
            )
            .is_ok()
        );
        assert!(
            resolve_mesh_old_material_channel(&property(BASE_COLOR, false), &bindings()).is_ok()
        );
    }
    ...
    fn constants_pass_and_computed_occlusion_is_rejected() {
        assert_eq!(
            resolve_mesh_old_material_channel(&MaterialChannel::One, &bindings()).unwrap(),
            MaterialChannel::One
        );
        assert!(
            resolve_mesh_old_material_channel(&MaterialChannel::ComputedOcclusion, &bindings())
                .is_err()
        );
    }
}
