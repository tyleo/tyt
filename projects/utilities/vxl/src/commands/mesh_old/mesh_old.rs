use crate::{
    ...
    commands::{
        MeshOldAtlas, MeshOldPropertyBinding, MeshOldTexture, MeshOldTextureArg, MeshOldTextureMap,
        MeshOldTextureName, mesh_old_computed_occlusion_unsupported, parse_mesh_old_atlas_shape,
    },
    require_file_name,
};
use clap::Parser;
...
use voxsmith::{
    ...
    operations::mesh_old::{AtlasShape, MaterialMap, MaterialMeshRequest, MeshMethod, mesh_old},
};
...
pub struct MeshOld {
    ...
    #[arg(
        ...
        long,
        default_value = "pot",
        value_parser = parse_mesh_old_atlas_shape,
...
impl MeshOld {
    pub fn execute(self, dependencies: impl Dependencies) -> Result<()> {
        ...
        // the palette atlas.
        if let MeshOldAtlas::Unwrap = self.atlas {
        ...
        // `mesh-old` outputs one object, so the selection must resolve to exactly
        ...
        let object_id = match object_ids.as_slice() {
            ...
            object_ids => {
                return Err(Error::usage(format!(
                    "the selection resolved to {} objects, but `mesh-old` outputs exactly one; \
        ...
        let document = mesh_old(&VoxsmithDependenciesImpl, &main, object, &request)?;
    ...
    fn resolve_presets(&self, stem: &str) -> Result<Vec<MaterialMap>> {
        ...
        let presets: Vec<MeshOldTexture> =
            self.texture.iter().flat_map(|arg| arg.textures()).collect();
        ...
        let texture_names = self
            .texture_name
            .chunks(2)
            .map(|pair| MeshOldTextureName::new(&pair[0], &pair[1]))
            .collect::<Result<Vec<_>>>()?;

        let mut names: HashMap<MeshOldTexture, String> = HashMap::new();
        ...
        let baked: HashSet<MeshOldTexture> = presets.iter().copied().collect();
        for texture_name in &texture_names {
        ...
        presets
            .into_iter()
            .map(|preset| {
                if let MeshOldTexture::ComputedOcclusion = preset {
                    return Err(mesh_old_computed_occlusion_unsupported());
                }
    ...
    fn preset_name(
        &self,
        names: &HashMap<MeshOldTexture, String>,
    ...
    fn resolve_custom_maps(&self) -> Result<Vec<MaterialMap>> {
        ...
        let maps = self
            .texture_map
            .chunks(2)
            .map(|pair| MeshOldTextureMap::new(&pair[0], &pair[1]))
            .collect::<Result<Vec<_>>>()?;
    ...
    fn property_bindings(&self) -> Result<Vec<MeshOldPropertyBinding>> {
        self.define_property
            .chunks(2)
            .map(|pair| MeshOldPropertyBinding::new(&pair[0], &pair[1]))
            .collect()
    }
...
mod tests {
    ...
    fn names(args: &[&str], output: &str) -> Vec<String> {
        let mut argv = vec!["mesh-old", "model.vox"];
        argv.extend_from_slice(args);
        MeshOld::try_parse_from(argv)
            .unwrap()
    ...
    fn the_texture_shape_defaults_to_pot_and_parses_keywords_and_a_pixel_size() {
        let default = MeshOld::try_parse_from(["mesh-old", "model.vox"]).unwrap();
        ...
        for (value, expected) in [
            ...
            let parsed =
                MeshOld::try_parse_from(["mesh-old", "model.vox", "--texture-shape", value])
                    .unwrap();
        ...
        assert!(
            MeshOld::try_parse_from(["mesh-old", "model.vox", "--texture-shape", "huge"]).is_err()
        );
        assert!(
            MeshOld::try_parse_from(["mesh-old", "model.vox", "--texture-shape", "0"]).is_err()
        );
    }

    #[test]
    fn hidden_variants_are_absent_from_help() {
        let command = MeshOld::command();
