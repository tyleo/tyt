...
/// Every palette has bindings with distinct attributes and in-range pool refs,
/// and column-major materials with one column per binding, a shared length M,
/// and value-indices within each bound pool. A palette with no bindings instead
/// stores one empty array per material, so M survives as `materials.length`.
pub fn check_palettes(main: &VoxjMain, failures: &mut Failures) {
    ...
    for (index, palette) in state.palettes.iter().enumerate() {
        ...
        // A binding-less palette has no columns; each material is an empty
        // array instead, and M is the entry count.
        if palette.bindings.is_empty() {
            check_binding_less_materials(index, palette, failures);
            continue;
        }
...
/// Every entry of a binding-less palette's `materials` is an empty array, one
/// per material, carrying no value-indices.
fn check_binding_less_materials(index: usize, palette: &VoxjPalette, failures: &mut Failures) {
    for (material, values) in palette.materials.iter().enumerate() {
        if !values.is_empty() {
            failures.report(
                Check::Palettes,
                format!(
                    "palette {index} has no bindings, so material {material} must be an empty \
                     array, not {} value-indices",
                    values.len()
                ),
            );
            if !failures.go() {
...
/// Every materials column shares the length M, and every value-index falls
/// within the values of the pool its binding names.
fn check_materials(
    index: usize,
    palette: &VoxjPalette,
