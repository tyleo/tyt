...
pub enum VoxjSampleBlock {
    /// One channel per sampled layer: that layer's material index for every
    /// voxel, in order.
    #[cfg_attr(feature = "serde", serde(rename = "raw-json"))]
    RawJson(Vec<Vec<u32>>),

    /// One run-length channel per sampled layer: a flat `[value, count, ...]`
    /// stream.
    #[cfg_attr(feature = "serde", serde(rename = "rle-json"))]
    RleJson(Vec<Vec<u32>>),

    /// One base64 bit-packed channel per sampled layer.
