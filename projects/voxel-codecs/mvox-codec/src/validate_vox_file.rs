...
/// [`to_vox_file_bytes`](crate::to_vox_file_bytes) would write as a structurally
/// valid but broken file. Decoding does not call this. Run it when you need the
/// guarantee.
pub fn validate_vox_file(file: &MVoxFile) -> Result<()> {
...
mod tests {
    ...
    fn accepts_a_valid_file() {
        assert!(validate_vox_file(&valid_file()).is_ok());
    }

    #[test]
    fn accepts_an_empty_file() {
        assert!(validate_vox_file(&MVoxFile::default()).is_ok());
    }
    ...
    fn rejects_a_voxel_outside_its_model() {
        ...
        assert!(validate_vox_file(&file).is_err());
    }
    ...
    fn accepts_the_no_layer_sentinel() {
        ...
        assert!(validate_vox_file(&file).is_ok());
    }
