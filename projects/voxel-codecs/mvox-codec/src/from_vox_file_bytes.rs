...
pub fn from_vox_file_bytes(bytes: &[u8]) -> Result<MVoxFile> {
...
#[cfg(test)]
mod tests {
    use crate::{from_vox_file_bytes, to_vox_file_bytes};
    use mvox::{
    ...
    fn round_trips_full_file() {
        ...
        let bytes = to_vox_file_bytes(&file);
        let decoded = from_vox_file_bytes(&bytes).unwrap();
    ...
    fn round_trips_empty_file() {
        ...
        let bytes = to_vox_file_bytes(&file);
        assert_eq!(from_vox_file_bytes(&bytes).unwrap(), file);
    }
    ...
    fn parses_hand_written_minimal_file() {
        ...
        let file = from_vox_file_bytes(&bytes).unwrap();
    ...
    fn rejects_bad_magic() {
        ...
        assert!(from_vox_file_bytes(bytes).is_err());
    }

    #[test]
    fn rejects_truncated_header() {
        assert!(from_vox_file_bytes(b"VOX ").is_err());
        assert!(from_vox_file_bytes(b"").is_err());
    }
    ...
    fn rejects_consecutive_size_chunks() {
        ...
        assert!(from_vox_file_bytes(&vox_file(&children)).is_err());
    }
    ...
    fn rejects_trailing_content_bytes() {
        ...
        assert!(from_vox_file_bytes(&vox_file(&chunk(b"SIZE", &content))).is_err());
    }
