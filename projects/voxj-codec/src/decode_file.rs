...
pub fn decode_file(file: &VoxjSerdeFile) -> io::Result<VoxjCodecFile> {
    ...
    let objects = file
        ...
        .map(|object| {
            ...
            decode_object(object, &cell_counts)
        })
...
#[cfg(test)]
mod tests {
    use crate::{PositionEncoding, SampleEncoding, decode_file, encode_file, encode_file_smallest};
    ...
    fn round_trips_file_through_fixed_encoding() {
        ...
        assert_round_trip(&decode_file(&encoded).unwrap());
    }

    #[test]
    fn round_trips_file_through_smallest() {
        let encoded = encode_file_smallest(&codec_file());
        assert_round_trip(&decode_file(&encoded).unwrap());
    }
}
