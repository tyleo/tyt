use crate::{
    Dependencies, Error, MeshInput, MeshOutput, MeshOutputDone, MeshProcessed, MeshRequest,
    MeshTask, MeshTaskFile, Model, ModelType, Result, TargetFormat, TextureQuality, Topology,
};
...
/// Polls the task until it succeeds, fails, or the timeout elapses.
fn poll_task(
    ...
    let mut waited = 0u64;
    loop {
        let task = dependencies.get_task(api_key, task_id)?;
