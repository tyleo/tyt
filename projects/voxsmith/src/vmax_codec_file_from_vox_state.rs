...
use vmax::{
    VMaxCodecContentsVmaxbFile, VMaxCodecFile, VMaxCodecMaterial,
    VMaxCodecPaletteSettingsVmaxpsbFile, VMaxCodecVoxel, VMaxGroup, VMaxMaterialDispersion,
    VMaxObject, VMaxPalettePngFile,
};
use voxcore::{
...
/// Writes a [`VoxState`] back to a fully decoded Voxel Max document, the inverse
/// of [`vox_state_from_vmax_codec_file`](crate::vox_state_from_vmax_codec_file).
...
pub fn vmax_codec_file_from_vox_state(
    state: &VoxState,
    voxel_max_color_format: VoxelMaxColorFormat,
) -> Result<VMaxCodecFile> {
    ...
    let mut contents_files: BTreeMap<String, VMaxCodecContentsVmaxbFile> = BTreeMap::new();
    let mut palette_settings_files: BTreeMap<String, VMaxCodecPaletteSettingsVmaxpsbFile> =
        BTreeMap::new();
        ...
        let data = match contents_by_object.get(&object_id) {
            ...
            None => {
                ...
                let contents = match voxel_max
                    ...
                    .and_then(|s| s.clone())
                {
                    Some(state) => VMaxCodecContentsVmaxbFile {
                        voxels,
                        uuid: state.uuid,
                        v: state.v,
                        tools: state.tools,
                    ...
                    None => VMaxCodecContentsVmaxbFile {
                        voxels,
    ...
    Ok(VMaxCodecFile {
        ...
        quick_look_png_files: BTreeMap::new(),
    })
}
...
fn build_palette(
    ...
    palette_settings_files: &mut BTreeMap<String, VMaxCodecPaletteSettingsVmaxpsbFile>,
...
/// Builds a decoded material palette from a voxcore palette's cells, reading the
...
fn material_settings(
    ...
    name: String,
    colors: Vec<[u8; 4]>,
) -> VMaxCodecPaletteSettingsVmaxpsbFile {
    let palette = state.palette(palette).expect("a referenced palette");
    ...
    let materials = palette
        ...
        .map(|cell| {
            ...
            VMaxCodecMaterial {
                metalness: number("metallic", 0.0),
                roughness: number("roughness", 0.0),
                emission: number("emissive", 0.0),
                enable_shadows: matches!(value("shadows"), Some(VoxValue::Bool(true))),
                dispersion: dispersed.then(|| VMaxMaterialDispersion {
...
mod tests {
    use crate::{
        VoxelMaxColorFormat, vmax_codec_file_from_vox_state, vox_state_from_vmax_codec_file,
    };
    use std::collections::BTreeMap;
    use vmax::{
        VMaxCodecContentsVmaxbFile, VMaxCodecFile, VMaxCodecMaterial,
        VMaxCodecPaletteSettingsVmaxpsbFile, VMaxCodecVoxel, VMaxGroup, VMaxObject,
        VMaxPalettePngFile, VMaxSceneCamera, VMaxSceneJsonFile,
    };
    ...
    fn palette_settings(
        name: &str,
        materials: Vec<VMaxCodecMaterial>,
        colors: Vec<[u8; 4]>,
    ) -> VMaxCodecPaletteSettingsVmaxpsbFile {
        VMaxCodecPaletteSettingsVmaxpsbFile {
            name: name.to_owned(),
    ...
    fn sample() -> VMaxCodecFile {
        ...
        let contents = VMaxCodecContentsVmaxbFile {
            voxels: vec![
                VMaxCodecVoxel {
                    position: [0, 0, 0],
                    material_idx: 1,
        ...
        palette_settings_files.insert(
            ...
            palette_settings(
                "mat",
                vec![
                    material(0.0, 1.0, 0.0, true),
                    material(0.5, 0.25, 2.0, false),
                ],
                Vec::new(),
            ),
        );
    ...
    fn round_trips_through_vox_state() {
        let original = sample();
        let state = vox_state_from_vmax_codec_file(&original).unwrap();
        let rebuilt = vmax_codec_file_from_vox_state(&state, VoxelMaxColorFormat::Png).unwrap();
    ...
    fn errors_without_voxel_max_ext() {
        ...
        assert!(vmax_codec_file_from_vox_state(&state, VoxelMaxColorFormat::Png).is_err());
    }
}
