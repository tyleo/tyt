...
pub fn vox_state_from_mvox_file(file: &MVoxFile) -> Result<VoxState> {
...
mod tests {
    use crate::{
        mvox_bytes_from_vox_state, mvox_file_from_vox_state, vox_state_from_mvox_bytes,
        vox_state_from_mvox_file,
    };
    use mvox::{
    ...
    fn round_trips_through_vox_state() {
        ...
        let state = vox_state_from_mvox_file(&file).unwrap();
        assert_files_eq(&mvox_file_from_vox_state(&state).unwrap(), &file);
    }
    ...
    fn round_trips_through_mvox_bytes() {
        let file = sample_file();
        let state = vox_state_from_mvox_file(&file).unwrap();
        let bytes = mvox_bytes_from_vox_state(&state).unwrap();
        let reloaded = vox_state_from_mvox_bytes(&bytes).unwrap();
        assert_files_eq(&mvox_file_from_vox_state(&reloaded).unwrap(), &file);
    }
    ...
    fn round_trips_the_empty_file() {
        ...
        let state = vox_state_from_mvox_file(&file).unwrap();
        assert_eq!(mvox_file_from_vox_state(&state).unwrap(), file);
    }
    ...
    fn round_trips_a_file_without_a_palette() {
        ...
        let state = vox_state_from_mvox_file(&file).unwrap();
        let rebuilt = mvox_file_from_vox_state(&state).unwrap();
    ...
    fn errors_without_magica_voxel_ext() {
        ...
        assert!(mvox_file_from_vox_state(&state).is_err());
    }
    ...
    fn rejects_a_material_id_outside_the_palette_range() {
        ...
        assert!(vox_state_from_mvox_file(&file).is_err());
    }
