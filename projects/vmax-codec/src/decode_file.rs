use crate::{decode_contents, decode_palette_settings};
...
/// [`encode_file`](crate::encode_file). The scene graph and `palette*.png` color
/// tables carry over unchanged; each `contents*.vmaxb` and
/// `palette*.settings.vmaxpsb` payload is decoded.
pub fn decode_file(file: &VMaxSerdeFile) -> Result<VMaxCodecFile> {
    let contents_files = file
        .contents_files
        .iter()
        .map(|(name, contents)| (name.clone(), decode_contents(contents)))
        .collect();
    let palette_settings_files = file
        .palette_settings_files
        .iter()
        .map(|(name, palette)| Ok((name.clone(), decode_palette_settings(palette)?)))
...
#[cfg(test)]
mod tests {
    use crate::{decode_file, encode_file, encode_snapshots};
    ...
    /// A raw serde package already in the canonical form `encode_file` produces:
    ...
    /// `decode_file` then `encode_file` reproduces the original serde package: the
    ...
    fn round_trips_serde_file_through_codec() {
        ...
        let decoded = decode_file(&original).unwrap();
        ...
        assert_eq!(encode_file(&decoded), original);
    }
}
