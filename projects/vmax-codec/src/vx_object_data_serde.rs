...
impl VXObjectDataSerde {
    /// Decodes every voxel into model space, replaying snapshots so the last
    /// snapshot of each chunk wins. Returns voxels sorted by `(x, y, z)`.
    pub fn voxels(&self) -> Vec<VMaxVoxel> {
        // Keep the last snapshot per chunk (later entries overwrite earlier).
        let mut latest: BTreeMap<u32, &VXSnapshotSerde> = BTreeMap::new();
        ...
        voxels
            .into_iter()
            .map(|((x, y, z), (material, color))| VMaxVoxel {
                x,
                y,
                z,
                material,
