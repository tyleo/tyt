use crate::ext::{VMaxExt, VMaxExtNode, VMaxExtObjectState, VMaxExtPalette};
use vmax::{VMaxContentsVmaxbFile, VMaxFile, VMaxGroup, VMaxObject};
use voxcore::{VoxExt, VoxMain};

/// Puts the ext a read yields on the bare state. [`VMaxExt`] keeps it. `()`
/// drops it.
pub trait VMaxExtSink: VoxExt + Sized {
    /// Puts on `state` the ext of a read of `serde`. `palettes` holds each
    /// folded palette's provenance in palette listing order. `object_data`
    /// holds each object's contents filename in object listing order.
    fn record_file(
        state: VoxMain<()>,
        serde: &VMaxFile,
...
impl VMaxExtSink for VMaxExt {
    fn record_file(
        ...
        let mut hierarchy_nodes: Vec<VMaxExtNode> =
            scene.groups.iter().map(node_from_group).collect();
...
impl VMaxExtSink for () {
    fn record_file(
        state: VoxMain<()>,
        _serde: &VMaxFile,
        _palettes: Vec<VMaxExtPalette>,
        _object_data: Vec<Option<String>>,
    ) -> VoxMain<Self> {
        state
    }
}
