...
/// Loads a Goxel [`GoxlFile`] into a bare [`VoxMain`]. The shared `BL16`
/// voxel blocks become objects sharing one `baseColor` palette, and the
/// `LAYR` layers become the hierarchy nodes placing them. The rest of the
/// Goxel state is dropped, so [`to_goxl_file`](crate::to_goxl_file) writes
/// the state back as a synthesized file. The `ext` feature's
/// `ext::from_goxl_file_with_ext` keeps that state instead.
///
