...
/// rebuilds exactly: each object emits one `16 x 16 x 16` block and the rest
...
/// Synthesizes a Goxel file from the bare scene of a state written without a
...
/// other alpha is kept.
fn synthesize_goxl<T>(state: &VoxMain<T>) -> Result<GoxlFile> {
