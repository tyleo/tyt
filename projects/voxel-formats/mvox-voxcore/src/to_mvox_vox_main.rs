...
pub fn to_mvox_vox_main(mut main: VoxMain<()>) -> Result<MVoxVoxMain> {
    check_grids(&main)?;

    merge_palettes(&mut main)?;

    let scene_nodes = rebuild_hierarchy(&mut main)?;
...
fn check_grids(main: &VoxMain<()>) -> Result<()> {
...
fn merge_palettes(main: &mut VoxMain<()>) -> Result<()> {
    ...
    for (_, object) in main.iter_objects() {
        let Some(cell_color) = resolve_cell_color(main, object)? else {
            continue;
        };
...
/// and returns the entries keyed by the new nodes. Every node becomes a
/// transform node carrying its translation over a group of its child nodes
...
fn rebuild_hierarchy(
    main: &mut VoxMain<()>,
) -> Result<BTreeMap<U32Id<BVoxHierarchyNode>, MVoxExtNode>> {
    let old: HashMap<U32Id<BVoxHierarchyNode>, VoxHierarchyNode> = main
        .iter_hierarchy_nodes()
        .map(|(node_id, node)| (node_id, node.clone()))
        .collect();
    let root_ids = main.root_hierarchy_node_ids().to_vec();
    ...
    let root_transform =
        builder.emit(SceneNodeKind::Transform, transform_vox_node([0, 0, 0], ""))?;
    let root_group = builder.emit(SceneNodeKind::Group, VoxHierarchyNode::default())?;
    builder.link(root_transform, vec![root_group])?;
...
/// Retains the rebuilt nodes and builds their entries. A node is retained
/// before its children, which puts parents before children in the scene-node
...
impl<'a> Builder<'a> {
    /// Retains `node` as a scene node of `kind` and records its entry.
    fn emit(
        &mut self,
        kind: SceneNodeKind,
        node: VoxHierarchyNode,
    ) -> Result<U32Id<BVoxHierarchyNode>> {
        let node_id = self.main.retain_hierarchy_node(node)?;
        let node = self
            .main
            .hierarchy_node(node_id)
            .expect("a retained node is listed");
        insert_synthesized_scene_node(&mut self.entries, node_id, kind, node);
        Ok(node_id)
    }

    /// Makes `child_ids` the child nodes of `node_id`.
    fn link(
        &mut self,
    ...
    /// subtree, returning the transform for a parent group to list.
    fn emit_node(&mut self, old_id: U32Id<BVoxHierarchyNode>) -> Result<U32Id<BVoxHierarchyNode>> {
        ...
        let group = self.emit(SceneNodeKind::Group, VoxHierarchyNode::default())?;
        self.link(transform, vec![group])?;
...
mod tests {
    ...
    use voxcore::{
        BVoxHierarchyNode, BVoxMaterial, BVoxObject, BVoxPalette, VoxExt, VoxHierarchyNode,
    ...
    /// A default main has no objects, so the synthesized file is empty.
    #[test]
    fn synthesizes_an_empty_state() {
        let main = to_mvox_vox_main(VoxMain::default()).unwrap();

        let file = to_mvox_file(&main).unwrap();
    ...
    /// A bare main, such as one cross-loaded from another format,
    ...
    fn synthesizes_a_file_from_a_bare_state() {
        let main = to_mvox_vox_main(source_main()).unwrap();
        let file = to_mvox_file(&main).unwrap();
        ...
        // The synthesized palette and scene graph read back into a valid main.
    ...
    /// each placement's transform and shape. Names stay on the nodes.
    #[test]
    fn synthesizes_an_entry_per_scene_node() {
        ...
        assert_eq!(main.hierarchy_node_count(), 12);

        assert_eq!(ext.scene_nodes.len(), 12);

        let nodes: Vec<_> = main.iter_hierarchy_nodes().collect();

        let entry = |index: usize| &ext.scene_nodes[&nodes[index].0];

        // A node's name rides its transform. The root, the groups, the
        // placements of nameless objects, and the shapes carry none.
        let names: Vec<&str> = nodes.iter().map(|(_, node)| node.name.as_str()).collect();
        ...
        // The ids count up in emission order, parents before children.
        for index in 0..12 {
            assert_eq!(entry(index).id, index as i32);
        }

        assert!(matches!(entry(0).body, MVoxExtNodeBody::Transform { .. }));

        assert_eq!(entry(1).body, MVoxExtNodeBody::Group);

        assert_eq!(nodes[0].1.child_node_ids, [nodes[1].0]);
        ...
        assert_eq!(models[0].object, U32Id::<BVoxObject>::from_u32(0));

        let MVoxExtNodeBody::Shape { models } = &entry(11).body else {
            panic!("a shape entry");
        };

        assert_eq!(models[0].object, U32Id::<BVoxObject>::from_u32(1));

        assert_eq!(main.root_hierarchy_node_ids(), [nodes[0].0]);

        // The written file lists the shapes' models by object listing index.
        let file = to_mvox_file(&main).unwrap();

        let models: Vec<u32> = file
            .scene_nodes
            .iter()
    ...
    fn merges_palettes_into_one_with_samples_remapped() {
        ...
        let red_palette_id = single_color_palette(&mut main, "#FF0000FF");

        let blue_palette_id = single_color_palette(&mut main, "#0000FFFF");

        let red_id = main.retain_object(unit_object(red_palette_id)).unwrap();

        let blue_id = main.retain_object(unit_object(blue_palette_id)).unwrap();

        let before: Vec<[u8; 4]> = [red_id, blue_id]
            .iter()
            .map(|&object_id| color_of(&main, object_id))
            .collect();

        let main = to_mvox_vox_main(main).unwrap();
    ...
    fn places_an_unplaced_object_under_the_root() {
        ...
        let palette_id = single_color_palette(&mut main, "#FF0000FF");

        main.retain_object(unit_object(palette_id)).unwrap();

        let main = to_mvox_vox_main(main).unwrap();

        assert_eq!(main.hierarchy_node_count(), 4);

        let (_, group) = main.iter_hierarchy_nodes().nth(1).unwrap();

        assert_eq!(group.child_node_ids.len(), 1);

        let (_, shape) = main.iter_hierarchy_nodes().nth(3).unwrap();

        assert_eq!(shape.child_object_ids.len(), 1);
    }

    /// A grid past 256 voxels on an axis has no byte coordinate for its far
    /// cells, so the main is refused.
    #[test]
    fn errors_on_a_grid_over_the_axis_limit() {
        let mut main = VoxMain::default();

        main.retain_object(VoxObject::new(String::new(), TyVector3U32::new(257, 1, 1)).unwrap())
            .unwrap();

        assert!(to_mvox_vox_main(main).is_err());
    }
    ...
    fn errors_past_the_palette_limit() {
        ...
        let value_pool_id = main.retain_value_pool(VoxValuePool::vec_4_float(colors).unwrap());
    ...
    fn single_color_palette(main: &mut VoxMain<()>, hex: &str) -> U32Id<BVoxPalette> {
        let value_pool_id =
            main.retain_value_pool(VoxValuePool::vec_4_float(vec![linear_rgba(hex)]).unwrap());
    ...
    fn color_of<T: VoxExt>(main: &VoxMain<T>, object_id: U32Id<BVoxObject>) -> [u8; 4] {
        let object = main.object(object_id).unwrap();

        let voxel_id = object.voxel_id(TyVector3U32::ZERO).unwrap();

        resolve_cell_color(main, object)
            .unwrap()
