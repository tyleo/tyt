...
/// Writes a state to a decoded MagicaVoxel [`MVoxFile`]. With `mvox_ext`, a
/// loaded file rebuilds exactly: each object emits one model and the rest
/// comes from the ext. Without it, `synthesize_mvox` builds the file from the
...
/// Synthesizes a MagicaVoxel file from the bare scene of a state written
/// without an `mvox` ext, such as one loaded from another format. Builds one
/// model per object, a single global 256-color palette gathering every
/// distinct color used, and a scene graph mirroring the voxcore hierarchy.
///
...
fn synthesize_mvox<T>(state: &VoxMain<T>) -> Result<MVoxFile> {
