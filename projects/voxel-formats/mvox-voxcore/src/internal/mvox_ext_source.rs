...
use voxcore::{
    BVoxHierarchyNode, BVoxMaterial, BVoxObject, BVoxPalette, VoxExt, VoxMain, VoxObject,
    color::{resolve_cell_color, resolve_cell_color_or_transparent, value_pool_color},
    material::BASE_COLOR,
};

/// Writes a state to a MagicaVoxel file. [`MVoxExt`] rebuilds the loaded
/// file. `()` synthesizes one from the scene.
pub trait MVoxExtSource: VoxExt + Sized {
    /// Writes `state` to an [`MVoxFile`].
    fn write_mvox(state: &VoxMain<Self>) -> Result<MVoxFile>;
}

impl MVoxExtSource for MVoxExt {
    /// Writes a state to a decoded MagicaVoxel [`MVoxFile`] through its ext, so
    /// a loaded file rebuilds exactly: each object emits one model and the rest
    /// comes from the ext. A model lists its voxels in ascending raster order,
    /// which need not match their original stored order.
    ///
    /// Errors if the ext's scene nodes are out of step with the hierarchy, or
    /// if a model exceeds a MagicaVoxel limit such as the per-axis voxel cap.
    fn write_mvox(state: &VoxMain<Self>) -> Result<MVoxFile> {
        let ext = state.ext();

        // The forward path adds exactly one palette and references it from
        // every object on one layer. Each material's color resolves through its
        // value pool.
...
impl MVoxExtSource for () {
    /// Synthesizes a MagicaVoxel file from the bare scene of a state carrying
    /// no `mvox` ext, such as one loaded from another format. Builds one model
    /// per object, a single global 256-color palette gathering every distinct
    /// color used, and a scene graph mirroring the voxcore hierarchy.
    ///
    /// Lossy only at MagicaVoxel's limits. An object grid may be at most 256
    /// voxels per axis because voxel coordinates are bytes. Node rotation and
    /// scale drop because a scene transform carries only translation. The
    /// palette is lossless up to 255 distinct colors. Beyond that the table is
    /// full and the excess collapses onto the last slot.
    fn write_mvox(state: &VoxMain<Self>) -> Result<MVoxFile> {
...
fn synthesize_palette<T>(state: &VoxMain<T>) -> Result<(MVoxPalette, HashMap<[u8; 4], u8>)> {
    let mut colors = [MVoxColor::default(); 256];
...
/// placement.
fn synthesize_scene<T>(state: &VoxMain<T>) -> Vec<MVoxSceneNode> {
...
impl SceneBuilder {
    ...
    fn emit_node<T>(&mut self, state: &VoxMain<T>, node_id: U32Id<BVoxHierarchyNode>) -> i32 {
