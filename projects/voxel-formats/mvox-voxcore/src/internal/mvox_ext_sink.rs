...
/// Puts the ext a read yields on the bare state. [`MVoxExt`] keeps it. `()`
/// drops it.
pub trait MVoxExtSink: VoxExt + Sized {
    /// Puts on `state` the ext of a read of `file`.
    fn record_file(state: VoxMain<()>, file: &MVoxFile) -> VoxMain<Self>;
}

impl MVoxExtSink for MVoxExt {
    fn record_file(state: VoxMain<()>, file: &MVoxFile) -> VoxMain<Self> {
        state.put_ext(MVoxExt {
...
impl MVoxExtSink for () {
    fn record_file(state: VoxMain<()>, _file: &MVoxFile) -> VoxMain<Self> {
        state
    }
}
