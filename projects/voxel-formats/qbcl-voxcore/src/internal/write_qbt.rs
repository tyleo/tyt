...
/// Writes a state to a decoded Qubicle Binary Tree [`QbtFile`]. The file
/// rebuilds only from `qbt_ext`: the scene tree is walked from the single
/// root, each matrix or compound object emitting its grid with the visibility
/// masks from the ext and the color from the palette.
///
/// Errors if:
///
/// 1. the ext is absent
