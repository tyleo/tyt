...
/// Writes a state to a Qubicle Construction Library file. [`QbclExt`]
/// rebuilds the loaded file. `()` synthesizes one from the scene.
pub trait QbclExtSource: VoxExt + Sized {
    /// Writes `state` to a [`QbclFile`].
    fn write_qbcl(state: &VoxMain<Self>) -> Result<QbclFile>;
}

impl QbclExtSource for QbclExt {
    /// Writes a state to a decoded Qubicle Construction Library [`QbclFile`]
    /// through its ext, so a loaded file rebuilds exactly: the scene tree is
    /// walked from the single root, each matrix or compound object emitting its
    /// grid with the visibility masks from the ext and the color from the
    /// palette. A node retained after the load has no entry. It emits a node
...
impl QbclExtSource for () {
    /// Synthesizes a Qubicle file from the bare scene of a state carrying no
    /// `qbcl` ext, such as one cross-loaded from another format.
    ///
    ...
    ///
    /// Lossy only where Qubicle cannot represent the source. A model's
    /// transform chunk cannot carry translation, so a group node's placement is
    /// folded into the world position of its descendant matrices, summed down
    /// the hierarchy and rounded to whole voxels. Node rotation and scale are
    /// dropped. Colors stay per voxel with no palette merge. A color's alpha
    /// drops because a Qubicle voxel stores no alpha. Each matrix is pivoted at
    /// its grid origin. Its position is then the world coordinate of the
    /// object's min corner.
    fn write_qbcl(state: &VoxMain<Self>) -> Result<QbclFile> {
