use crate::{
    Result,
    ext::{QbtVoxMain, to_qbt_file_with_ext},
};
...
/// [`to_qbt_file_with_ext`] and the inverse of
/// [`from_qbt_bytes_with_ext`](crate::codec::from_qbt_bytes_with_ext).
pub fn to_qbt_bytes_with_ext<D: CompressZlib>(
    dependencies: &D,
    state: &QbtVoxMain,
) -> Result<Vec<u8>> {
    let file = to_qbt_file_with_ext(state)?;
...
mod tests {
    use crate::{
        ...
        ext::{from_qbt_file_with_ext, to_qbt_file_with_ext},
    };
    ...
    fn round_trips_through_qbt_bytes() {
        ...
        let bytes = to_qbt_bytes_with_ext(&DependenciesImpl, &state).unwrap();
        let reloaded = from_qbt_bytes_with_ext(&DependenciesImpl, &bytes).unwrap();
        assert_eq!(to_qbt_file_with_ext(&reloaded).unwrap(), file);
    }
}
