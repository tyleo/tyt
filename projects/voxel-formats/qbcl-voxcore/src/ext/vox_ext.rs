...
/// The nodes follow the hierarchy listing. A retained node takes no entry.
/// The writer fills it in like a synthesized node. The ext lists nothing per
/// object. The writer reaches a node's object through the hierarchy. The
/// masks do not follow the voxel hooks. A repaint retains a live voxel again
/// and fires the same hook as a new voxel, so the list cannot tell them
/// apart.
