...
/// Writes a [`VoxjVoxMain`] and its `ext` block to pretty-printed `.voxj`
/// JSON bytes, the bytes form of [`to_voxj_file_with_ext`].
pub fn to_voxj_pretty_bytes_with_ext<D: EncodeBase64 + CostVoxjObject + EncodeVoxjJson>(
    dependencies: &D,
    state: &VoxjVoxMain,
